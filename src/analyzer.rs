...
//
// The analyzer turns the parse tree into an abstract syntax tree. Its main responsibilities
// include:
//
//   - Computing the type of each value in the program.
//   - Assigning a unique name to every symbol in the program.
//   - Recording errors to be reported to the user.
//   - Simplifying some parse-tree syntactic constructions.
//   - Calculating the register usage for each expression.
//
// It works by recursively traversing the parse tree and transforming each node into an AST node.
// Many parse tree nodes have direct AST equivalents. Some have simpler equivalents, e.g. the parse
// tree `if` nodes can have multiple `else if` clauses but the AST `if` nodes only have `if` and
// `else` clauses. Some parse tree nodes are mapped to different AST nodes entirely, e.g. `and` and
// `or` binary expressions become `if-else` expressions.
//
// TODO(#151): The analyzer is currently responsible for setting the `stack_offset` field of all
// expressions that are used as function arguments. The logic to do this is brittle because the
// analyzer generates a lot of "synthetic" function calls for things that were not function calls
// in the original program, e.g. `my_list[my_index]` is turned into `venice_list_index(my_list,
// my_index)`. Every place where this happens has to remember to set the `stack_offset` of each
// argument. It would be better to have a separate pass to do this after generate the AST so that
// all call expressions can be handled in one place.
...
struct Analyzer {
    ...
    // The declared return type of the current function, so that the types of `return` statements
    // can be checked.
    current_function_return_type: Option<ast::Type>,
    errors: Vec<errors::VeniceError>,

    // A counter appended to names generated by the compiler to ensure they are unique. Analyzer
    // methods should use `claim_unique_name` rather than using it directly.
    unique_name_counter: u64,

    // The offset of the next available slot in the stack. Analyzer methods should use
    // `claim_stack_offset` to claim room for a new variable on the stack rather than using it
    // directly.
...
impl Analyzer {
    ...
    /// Returns a name starting with `prefix` that is guaranteed to be unique in the entire
    /// program.
    ...
    /// Returns an offset for a new stack variable.
    fn claim_stack_offset(&mut self, type_: &ast::Type) -> i32 {
        let ret = self.current_stack_offset;
        self.current_stack_offset -= type_.storage_size();
        ret
    }
...
/// Sets the `max_register_needed` field of each expression node in the program.
///
/// Note that this logic assumes there are an infinite number of registers available; spilling
/// registers onto the stack where necessary is not done until code generation.
...
/// A mapping from symbol names to symbol entries. Used for both values and types, which exist in
/// separate namespaces.
struct SymbolTable {
    /// The stack of scopes, where the last element in the vector is the current scope, the
    /// next-to-last element is the current scope's parent, etc.
    scopes: Vec<HashMap<String, ast::SymbolEntry>>,
}
...
impl SymbolTable {
    ...
    /// Returns a symbol table populated with entries for the built-in types.
    ...
    /// Returns a symbol table populated with entries for the built-in globals.
    pub fn builtin_globals() -> Self {
        // Because the analyzer always generates names of the form {prefix}__{counter}, the unique
        // names used for runtime functions where (e.g., `venice_println`) can never conflict with
